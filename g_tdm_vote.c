...
void TDM_ApplyVote(void) {
    ...
    if (vote.flags & VOTE_ITEM_RANDOMIZER) {
        sprintf(value, "%d", vote.item_randomizer);
        g_item_randomizer = gi.cvar_set("g_item_randomizer", value);
    }

    if (vote.flags & VOTE_FAT_SKINS) {
        sprintf(value, "%d", vote.fat_skins);
        g_fat_skins = gi.cvar_set("g_fat_skins", value);
    }
...
static void TDM_AnnounceVote(void) {
    ...
    if (vote.flags & VOTE_ITEM_RANDOMIZER) {
        ...
        if (vote.item_randomizer) {
            strcat(what, "enable item randomizer");
        } else {
            strcat(what, "disable item randomizer");
        }
    }
    ...
    if (vote.flags & VOTE_FAT_SKINS) {
        ...
        if (vote.fat_skins) {
            strcat(what, "enable fat skins");
        } else {
            strcat(what, "disable fat skins");
        }
    }
...
/**
 * Vote to enable item randomizer (shuffle item and weapon positions at match start)
 */
qboolean TDM_VoteItemRandomizer(edict_t *ent) {
    int ir;
    const char *value;

    if (!((int) g_vote_mask->value & VOTE_ITEM_RANDOMIZER)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for item randomizer is not allowed on this server.\n");
        return false;
    }
    ...
    if (!Q_stricmp(value, "0")) {
        ...
        gi.cprintf(ent, PRINT_HIGH, "Unknown item randomizer setting: %s\n",
                value);
        return false;
    }

    if ((int) g_item_randomizer->value == ir) {
        gi.cprintf(ent, PRINT_HIGH,
                "Item randomizer is already %s!\n", ir ? "enabled" : "disabled");
        return false;
    }

    if (vote.active) {
        if (vote.item_randomizer == ir) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You've already started a vote for that item randomizer setting.\n");
            return false;
        }
    ...
    vote.flags |= VOTE_ITEM_RANDOMIZER;
    vote.item_randomizer = ir;

    return true;
}

/**
 * Vote to enable/disable fat skins (team aura effect).
 */
qboolean TDM_VoteFatSkins(edict_t *ent) {
    const char *value;
    int fs;

    if (!((int) g_vote_mask->value & VOTE_FAT_SKINS)
            && !ent->client->pers.admin) {
        gi.cprintf(ent, PRINT_HIGH,
                "Voting for fat skins is not allowed on this server.\n");
        return false;
    }
    ...
    if (!Q_stricmp(value, "0")) {
        ...
        gi.cprintf(ent, PRINT_HIGH, "Unknown fat skins setting: %s\n",
                value);
        return false;
    }

    if ((int) g_fat_skins->value == fs) {
        gi.cprintf(ent, PRINT_HIGH,
                "Fat skins is already %s!\n", fs ? "enabled" : "disabled");
        return false;
    }

    if (vote.active) {
        if (vote.fat_skins == fs) {
            gi.cprintf(ent, PRINT_HIGH,
                    "You've already started a vote for that fat skins setting.\n");
            return false;
        }
...
void TDM_Vote_f(edict_t *ent) {
    ...
    if (!Q_stricmp(gi.argv(0), "yes") || !Q_stricmp(gi.argv(0), "no")) {
        ...
        if (gi.argc() < 2) {
            ...
            gi.cprintf(ent, PRINT_HIGH, "Usage: vote <setting> <value>\n"
                    ...
                    "  itemrandomizer <0/1> (shuffle item and weapon positions)\n"
                    "  fatskins <0/1> (team aura effect)\n");
            return;
        }
    ...
    } else if (!Q_stricmp(cmd, "itemrandomizer")) {
        started_new_vote = TDM_VoteItemRandomizer(ent);
    } else if (!Q_stricmp(cmd, "fatskins")) {
        started_new_vote = TDM_VoteFatSkins(ent);
...
void TDM_VoteMenuApply(edict_t *ent) {
    ...
    if (ent->client->pers.votemenu_values.item_randomizer
            != (unsigned)(int) g_item_randomizer->value) {
        vote.item_randomizer = ent->client->pers.votemenu_values.item_randomizer;
        vote.flags |= VOTE_ITEM_RANDOMIZER;
        newvote = true;
    }

    if (ent->client->pers.votemenu_values.fat_skins
            != (unsigned)(int) g_fat_skins->value) {
        vote.fat_skins = ent->client->pers.votemenu_values.fat_skins;
        vote.flags |= VOTE_FAT_SKINS;
        newvote = true;
    }
