...
/**
 * Max entities to track for item/weapon position randomization
 */
#define MAX_RANDOMIZE_ENTS 128

/**
 * Check if an entity is a key item we want to shuffle:
 * armor_jacket, armor_combat, armor_body, mega health, backpack
 */
static qboolean IsRandomizableItem(edict_t *ent) {
    int idx;

    if (!ent->item)
        return false;

    idx = ITEM_INDEX(ent->item);
    if (idx == ITEM_ITEM_ARMOR_JACKET
            || idx == ITEM_ITEM_ARMOR_COMBAT
            || idx == ITEM_ITEM_ARMOR_BODY
            || idx == ITEM_ITEM_PACK)
        return true;

    // mega health: item points to generic health, check classname
...
/**
 * Check if an entity is a weapon we want to shuffle.
 * Excludes: blaster (not spawned), hand grenades (too many on maps)
 */
static qboolean IsRandomizableWeapon(edict_t *ent) {
    int idx;
    ...
    // exclude hand grenades (ammo_grenades has IT_WEAPON flag)
    if (idx == ITEM_AMMO_GRENADES)
        return false;

    // exclude blaster (shouldn't be on maps, but just in case)
    if (idx == ITEM_WEAPON_BLASTER)
        return false;
...
/**
 * Shuffle spawn positions of key items and weapons.
 * Items swap among item positions, weapons swap among weapon positions.
 * Uses Fisher-Yates shuffle on origin vectors.
 */
void TDM_RandomizeItemPositions(void) {
    vec3_t origins[MAX_RANDOMIZE_ENTS];
    edict_t *ents[MAX_RANDOMIZE_ENTS];
    int count;
    edict_t *ent;
    int i, j;
    vec3_t temp;

    // Phase 1: shuffle key items
    count = 0;
    for (ent = g_edicts + 1 + game.maxclients;
            ent < g_edicts + globals.num_edicts; ent++) {
        if (!ent->inuse)
        ...
        if (IsRandomizableItem(ent) && count < MAX_RANDOMIZE_ENTS) {
            gi.dprintf("  [item %d] cls=%s idx=%d at (%g %g %g)\n",
                    count, ent->classname, ITEM_INDEX(ent->item),
                    ent->s.origin[0], ent->s.origin[1], ent->s.origin[2]);
            ents[count] = ent;
            VectorCopy(ent->s.origin, origins[count]);
            count++;
        }
    }

    gi.dprintf("Item Randomizer: found %d key items\n", count);

    if (count > 1) {
        for (i = count - 1; i > 0; i--) {
            j = genrand_uniform(i + 1);
            VectorCopy(origins[i], temp);
            VectorCopy(origins[j], origins[i]);
            VectorCopy(temp, origins[j]);
        }

        for (i = 0; i < count; i++) {
            gi.dprintf("  [item %d] %s: moved to (%g %g %g)\n",
                    i, ents[i]->classname,
                    origins[i][0], origins[i][1], origins[i][2]);
            VectorCopy(origins[i], ents[i]->s.origin);
            droptofloor(ents[i]);
            if (!ents[i]->inuse) {
                gi.dprintf("  WARNING: %s was FREED by droptofloor!\n",
                        ents[i]->classname);
            }
        }
    }

    // Phase 2: shuffle weapons (excluding grenades)
    count = 0;
    for (ent = g_edicts + 1 + game.maxclients;
            ent < g_edicts + globals.num_edicts; ent++) {
        if (!ent->inuse)
        ...
        if (IsRandomizableWeapon(ent) && count < MAX_RANDOMIZE_ENTS) {
            ents[count] = ent;
            VectorCopy(ent->s.origin, origins[count]);
            count++;
        }
    ...
    gi.bprintf(PRINT_HIGH, "Item Randomizer active!\n");
    gi.dprintf("Item Randomizer: %d weapons shuffled\n", count);
}
...
void TDM_BeginMatch(void) {
    ...
    //randomize item/weapon positions if enabled
    if ((int)g_item_randomizer->value)
        TDM_RandomizeItemPositions();
...
void UpdateTeamMenu(void) {
    ...
    int ir = (int) g_item_randomizer->value;
    ...
    sprintf(openTDMBanner, "*Quake II - OpenTDM (%s%s)",
            ir ? "R" : "",
