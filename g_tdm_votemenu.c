...
void VoteMenuUpdate(edict_t *ent, unsigned flags) {
    if (flags & VOTE_MENU_GAMEMODE) {
        ...
        const char *base = gameString[ent->client->pers.votemenu_values.gamemode];
        int ir = ent->client->pers.votemenu_values.item_randomizer;
    ...
    if (flags & VOTE_MENU_ITEMRAND) {
        if (!((int) g_vote_mask->value & VOTE_ITEM_RANDOMIZER)) {
            ent->client->pers.votemenu[10].text = "Randomizer: disabled";
            ent->client->pers.votemenu[10].SelectFunc = NULL;
        } else {
            sprintf(ent->client->pers.votemenu_values.string_itemrand,
                    "Randomizer: %s",
                    ent->client->pers.votemenu_values.item_randomizer ? "ON" : "OFF");
            ent->client->pers.votemenu[10].text =
                    ent->client->pers.votemenu_values.string_itemrand;
            ent->client->pers.votemenu[10].SelectFunc = VoteMenuItemRandomizer;
        }
    }

    if (flags & VOTE_MENU_FATSKINS) {
        if (!((int) g_vote_mask->value & VOTE_FAT_SKINS)) {
            ent->client->pers.votemenu[14].text = "Fat Skins: disabled";
            ent->client->pers.votemenu[14].SelectFunc = NULL;
        } else {
            sprintf(ent->client->pers.votemenu_values.string_fatskins,
                    "Fat Skins: %s",
                    ent->client->pers.votemenu_values.fat_skins ? "ON" : "OFF");
            ent->client->pers.votemenu[14].text =
                    ent->client->pers.votemenu_values.string_fatskins;
            ent->client->pers.votemenu[14].SelectFunc = VoteMenuFatSkins;
        }
    }
}
...
/**
 * Vote menu item randomizer toggle handler
 */
void VoteMenuItemRandomizer(edict_t *ent) {
    ent->client->pers.votemenu_values.item_randomizer =
            !ent->client->pers.votemenu_values.item_randomizer;
    VoteMenuUpdate(ent, VOTE_MENU_ITEMRAND | VOTE_MENU_GAMEMODE);
    PMenu_Update(ent);
    gi.unicast(ent, true);
}

/**
 * Vote menu fat skins toggle handler
 */
void VoteMenuFatSkins(edict_t *ent) {
    ent->client->pers.votemenu_values.fat_skins =
            !ent->client->pers.votemenu_values.fat_skins;
    VoteMenuUpdate(ent, VOTE_MENU_FATSKINS);
    PMenu_Update(ent);
    gi.unicast(ent, true);
}
...
void OpenVoteMenu(edict_t *ent) {
    ...
    ent->client->pers.votemenu_values.decrease = false;
    ent->client->pers.votemenu_values.item_randomizer =
            (int) g_item_randomizer->value;
    ent->client->pers.votemenu_values.fat_skins =
            (int) g_fat_skins->value;
