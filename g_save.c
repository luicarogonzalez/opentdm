...
void InitGame(void) {
    ...
    // shuffle item and weapon positions at match start
    g_item_randomizer = gi.cvar("g_item_randomizer", "0", 0);

    // fat skins - team aura effect (red for team A, green for team B)
    g_fat_skins = gi.cvar("g_fat_skins", "0", 0);
